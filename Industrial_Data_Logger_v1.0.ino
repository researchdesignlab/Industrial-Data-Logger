  ...
  while (Serial.available()) 
  {
    ...
    if(ch == '$')
    {
      ...
      if (strstr(SMS, "AT+READ") != NULL)
      {
        ...
        token = strtok(NULL, "=");
        DATA_ch = atoi(token);     
        read_ee(DATA_ch); /* read from FRAM */
        F_UP = 0;
      }
      else /* it is a write command, so store the data into FRAM */
      {
        ...
        if (strstr(SMS, "AT+MBD") != NULL)
          Store(token, MODBUS_DATA); 
        else if (strstr(SMS, "AT+ADC") != NULL)
          Store(token,ADC_4TO20); 
 ...
 */ 
void Store( uint8_t values[],uint16_t framAddr)
{
 ...
 */
void UPLOAD_JISON(String values)
{
  unsigned char c;
  unsigned int filelength;
  DATA_UPDATE = 0;
